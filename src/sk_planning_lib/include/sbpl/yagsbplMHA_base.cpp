...
void HeapContainer<NodeType,CostType,PlannerSpecificVariables>::update (heapItem_p np, BubbleDirection bubdir)
{
    ...
    if (!(np->inHeap[id])) return;
    int currentPos = np->heapArrayPos[id];
//	ROS_INFO("HeapContainer::update(): id = %d, currentPos = %d.", id, currentPos);
    
    // Bubble up
    if (bubdir != DOWNONLY_BUBDIR) {
        int parentPos = (currentPos - 1) / 2; // position of parent
//        printf("id = %d, heapSize = %d, currentPos = %d == %d, parentPos = %d.\n", id, heap_size, currentPos, np->heapArrayPos[id], parentPos);
//        printf("compare if( %.3f > %.3f).\n", f_val(heapArray[parentPos]), f_val(np));
        if ( f_val(heapArray[parentPos]) > f_val(np) ) {
            heapArray[currentPos] = heapArray[parentPos];
            heapArray[parentPos] = np;
            heapArray[currentPos]->heapArrayPos[id] = currentPos;
            heapArray[parentPos]->heapArrayPos[id] = parentPos;
    ...
    if ( f_val(np) > f_val(heapArray[exchangeChildPos]) ) {
        ...
        heapArray[currentPos]->heapArrayPos[id] = currentPos;
        heapArray[exchangeChildPos]->heapArrayPos[id] = exchangeChildPos;
									...
									( SearchGraphNode<NodeType,CostType,PlannerSpecificVariables>* np )
{
    ...
    // Add as leaf
//    ROS_INFO("heap::push(): We push a node to heap %u.", id);
	if( np->inHeap[id] ) {
    ...
    np->inHeap[id] = true;
    np->heapArrayPos[id] = ((int)heapArray.size()) - 1;
    // Bubble up
    update(np, UPONLY_BUBDIR);
}
...
{
//	ROS_INFO("heap::pop(): We pop from heap %u, whose length is %zu.", id, heapArray.size());
	heapItem_p ret = heapArray[0];
    ret->inHeap[id] = false;
    ret->heapArrayPos[id] = -1;
    // Take last leaf and place it at root
    if( heapArray.size() > 1 ) {
	    heapArray[0] = heapArray[heapArray.size()-1];
    	heapArray[0]->heapArrayPos[id] = 0;
    }
    heapArray.pop_back();
    heap_size--;
    // Bubble down
...
{
	if( np->inHeap[id] ) {
//		ROS_INFO("heap::remove(): We remove from heap %u, whose length is %zu and its pos is %d.", id, heapArray.size(), np->heapArrayPos[id]);
		if( heapArray.size() > 1 ) {
			CostType tmp_f = np->m_f[id];
			np->m_f[id] = -std::numeric_limits<CostType>::infinity();
//			ROS_INFO("heap::remove(): we changfe the values and now update().");
			update(np);
//			ROS_INFO("heap::remove(): we changfe the values and now pop.");
			pop();
			np->m_f[id] = tmp_f;
		}
		else {
...
//	ROS_INFO("heap::remove(): Done.");
//	CostType tmp_h = np->h[id];
//	np->h[id] = -std::numeric_limits<CostType>::infinity();
//	update(np);
//	pop();
//	np->h[id] = tmp_h;
}
...
GenericSearchGraphDescriptor<NodeType,CostType>::GenericSearchGraphDescriptor()
{
	...
	storePath_fp = NULL;
	stopSearch_fp = NULL;
	deactivateSearch_fp = NULL;
	//incHeuristics_fp = NULL;
	func_container = NULL;
}
...
template <class NodeType, class CostType>
std::vector<CostType> GenericSearchGraphDescriptor<NodeType,CostType>::_getHeuristics(NodeType& n1, NodeType& n2)
{
	if (getHeuristics_fp)
	...
	{
		func_container->func_redefined = true;
		std::vector<CostType> ret = func_container->getHeuristics(n1, n2);
		if (func_container->func_redefined)
			return ret;
	}
...
template <class NodeType, class CostType>
std::vector<CostType> GenericSearchGraphDescriptor<NodeType,CostType>::_getHeuristicsToTarget(NodeType& n)
{
...
template <class NodeType, class CostType>
int GenericSearchGraphDescriptor<NodeType,CostType>::_deactivateSearch(NodeType& n)
{
	if (deactivateSearch_fp)
		return ( deactivateSearch_fp(n) );
	
	if (func_container)
	{
		func_container->func_redefined = true;
		bool ret = func_container->deactivateSearch(n);
		if (func_container->func_redefined)
			return ret;
	}
...
/*template <class NodeType, class CostType>
int GenericSearchGraphDescriptor<NodeType,CostType>::_incHeuristics(NodeType& n)
{
	if (incHeuristics_fp)
		return ( incHeuristics_fp(n) );
	
	if (func_container)
	{
		func_container->func_redefined = true;
		bool ret = func_container->incHeuristics(n);
		if (func_container->func_redefined)
			return ret;
	}
...
template <class NodeType, class CostType, class PlannerSpecificVariables>
void HashTableContainer<NodeType,CostType,PlannerSpecificVariables>::init_HastTable(int hash_table_size, unsigned int number_Of_Heuristic)
{
	...
	numberOfHeuristic = number_Of_Heuristic;
}
...
{
	// Search in bin
//	ROS_INFO("getNodeInHash(): start.");
	int hashBin = friendGraphDescriptor_p->_getHashBin(n);
//	ROS_INFO("getNodeInHash(): hash is %d.", hashBin);
...
//	ROS_INFO("getNodeInHash(): It is not in the hashtable.");
	// If new node, create it!
	SearchGraphNode<NodeType,CostType,PlannerSpecificVariables>* newSearchGraphNode = 
															new SearchGraphNode<NodeType,CostType,PlannerSpecificVariables>(numberOfHeuristic);
//	ROS_INFO("getNodeInHash(): Create one.");
...
//	ROS_INFO("getNodeInHash(): Resize hash.");
	HashTable[hashBin].push_back(newSearchGraphNode);
//	ROS_INFO("getNodeInHash(): push it.");
...
void GenericPlanner<NodeType,CostType,PlannerSpecificVariables>::init
						( GenericSearchGraphDescriptor<NodeType,CostType> theEnv , int heapKeyNum, int numberOfHeap )
{
	...
	*GraphDescriptor = theEnv;
	...
	numberOfHeuristic = numberOfHeap;
	hash->init_HastTable( GraphDescriptor->hashTableSize, numberOfHeuristic );
	
	heap.resize(numberOfHeap);
	for( unsigned int j = 0; j < numberOfHeap; j++ ) {
		heap[j] = new HeapContainer<NodeType,CostType,PlannerSpecificVariables>(heapKeyNum);
		heap[j]->setId(j);
	}
}
