...
REDIS_KEY   = "gps_state"
FIX_TOPIC = os.getenv("FIX_TOPIC", "/fix")
FIX_INFO_TOPIC = os.getenv("FIX_INFO_TOPIC", "/fix_info") 
FIX_RATE_HZ = float(os.getenv("FIX_RATE_HZ", "10.0")) 
...
class GpsFixPublisher(Node):
    def __init__(self,
                 fix_topic: str = "/fix",
                 info_topic: str = "/fix_info",
                 rate_hz: float = 10.0):
        super().__init__("gps_fix_publisher")

        # NavSatFix는 센서성 데이터라 sensor QoS 쓰는 게 일반적으로 안정적
        self.pub_fix = self.create_publisher(NavSatFix, fix_topic, qos_profile_sensor_data)

        # 추가 정보(quality/hdop/carrsoln)를 같이 내보내고 싶으면 String(JSON)로 하나 더
        self.pub_info = self.create_publisher(String, info_topic, 10)

        self.timer = self.create_timer(1.0 / max(1e-3, rate_hz), self._on_timer)
    ...
    def _on_timer(self):
        ...
        quality, hdop, carr = get_rtk_state()
        ...
        fix.altitude = 0.0  # 고도 안 쓰면 0.0

        # status: "fix가 있냐 없냐" 정도만 표준적으로 표현 가능
        # - GGA quality 0이면 no-fix
        # - quality가 None이면 carrSoln으로 추정(없으면 no-fix)
        has_fix = False
        if quality is not None:
            has_fix = (int(quality) != 0)
        else:
            has_fix = (int(carr) != 0)
        ...
        # covariance는 HDOP만으로는 정확히 계산하기 애매해서 UNKNOWN 권장
        fix.position_covariance_type = NavSatFix.COVARIANCE_TYPE_UNKNOWN

        self.pub_fix.publish(fix)

        # 2) /fix_info (선택) : 품질/HDOP/carrSoln을 JSON으로 같이 발행
        info = {
            "stamp": time.time(),
            "lat": float(lat),
            "lon": float(lon),
            "gga_quality": None if quality is None else int(quality),
...
def start_ros_subscribers(go2_topic=GO2_TOPIC, cmd_topic=CMD_TOPIC, pub_rate=CMD_RATE_HZ):
    def _spin():
        ...
        fix_node = GpsFixPublisher(fix_topic=FIX_TOPIC, info_topic=FIX_INFO_TOPIC, rate_hz=FIX_RATE_HZ)
